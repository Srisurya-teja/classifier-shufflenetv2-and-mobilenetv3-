...
End-to-end fine-tuning of ShuffleNetV2 x0.5 backbone with dual classification heads.
...
from shufflenet_model import (ShuffleNetV2Backbone,
...
def main():
    ...
    # Fused single ONNX — backbone + heads in one file for production
    fused_path = os.path.join(args.save_dir, "classifier_full.onnx")
    dummy = torch.randn(1, 3, 112, 112)
    torch.onnx.export(
        model, dummy, fused_path,
        input_names=["input"],
        output_names=["face_logits", "mask_logits"],
        dynamic_axes={"input": {0: "batch"}},
        opset_version=18, dynamo=False,
    )
    fused_mb = os.path.getsize(fused_path) / 1e6
    logger.info(f"[export] Fused    → {fused_path}  ({fused_mb:.2f} MB)")

    logger.info(f"\nProduction files:")
    logger.info(f"  {args.save_dir}/classifier_full.onnx   <- fused (benchmark + production)")
    logger.info(f"  {args.save_dir}/backbone.onnx          <- separate backbone")
    logger.info(f"  {args.save_dir}/classifier_heads.onnx  <- separate heads")
